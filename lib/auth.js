...
export const authOptions = {
  providers: [
    CredentialsProvider({
      name: "Credentials",
      credentials: {
        identifier: { label: "Roll Number or Admin Username", type: "text" },
      ...
      async authorize(credentials, req) {
        ...
        // Demo Admin Console Login Logic
        if (credentials.isAdmin === 'true' || credentials.isFaculty === 'true') {
          if (credentials.identifier === 'admin' && credentials.password === 'admin123') {
            return { id: 'admin-1', name: 'Professor Admin', role: 'admin' };
          }
          return null;
        }
